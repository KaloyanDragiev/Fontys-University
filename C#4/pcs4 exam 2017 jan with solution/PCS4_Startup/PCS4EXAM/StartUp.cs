...
namespace PCS4EXAM
{
    public class StartUp : IComparable<StartUp>
    {
        ...
        public int getSumOfDonationsAbove(int limit, int n)
        {
            ...
            {
                int temp = getSumOfDonationsAbove(limit, n - 1);
                if (limit < this.donations[n-1].Amount)
                {
                    return temp + this.donations[n - 1].Amount;
                }
                else { return temp; }
            }
        }
        ...
        {
            return this.getNrOfDonations() - other.getTotalOfDonations();
        }
    }
}
